...
(function () {
    ...
    function makePegRowDom_(cfg) {
        ...
        function updateStatement_() {
            ...
            // Favor lines (computed from the same totals as BUY/SELL lines)
            const soldBuyTotal = (soldBuy != null) ? (leftQtyInd * soldBuy) : null;
            const soldSellTotal = (soldSell != null) ? (leftQtyInd * soldSell) : null;
            const pegBuyTotal = (pegBuy != null) ? (rightQtyInd * pegBuy) : null;
            ...
            // If sold item is not in catalog, show missing catalog price (per requirement)
            const soldIt = O.findCatalogItem(soldName);
            if (!soldIt && String(soldName || '').trim()) {
            ...
            const lines = [
                esc(eqLine),
                `<span class="muted">${esc(buyLine)}</span>`,
                `<span class="muted">${esc(sellLine)}</span>`
            ];
            if (customerFavorHtml) lines.push(customerFavorHtml);
            if (merchantFavorHtml) lines.push(merchantFavorHtml);
            statement.innerHTML = lines.join('<br>');
        }
    ...
    // Expose legacy global names too (back-compat with boot/other modules)
    ...
    // Back-compat: also attach to window so older code that calls window.initCreatorPegUIs_ / window.addAltPeg_ still works
    // This fixes errors like "O.initCreatorPegUIs_ is not a function" when some code expects global functions.
    window.makePegRowDom_ = makePegRowDom_;
    window.validatePegSet_ = validatePegSet_;
    window.buildPegPayload_ = buildPegPayload_;
    window.initCreatorPegUIs_ = initCreatorPegUIs_;
    window.addAltPeg_ = addAltPeg_;
})();