...
(function () {
  ...
  const PUBLIC_PRICE_CATALOG_ID =
    "1_meliJtuKSDwEWRDh1gldcsD-pSjDgIND3dcE1mCjCo";
  ...
  async function loadPolicies() {
    ...
    const r = await window.apiGet("insuranceListMy", {
  ...
  async function loadPriceItems_() {
    try {
      ...
      const items = rows
        .map((r) => {
          const c = (i) =>
            r.c && r.c[i] && r.c[i].v !== undefined ? r.c[i].v : null;
    ...
    for (const [metal, pct] of Object.entries(allocObj)) {
      ...
      const nuggetItem =
        metal !== "Steel"
          ? items.find(
              (pi) =>
                pi.name.includes(metal + " Nuggets") ||
                pi.name.includes(metal + " Nugget"),
            )
          : null;
