...
<div class="alloc-row">
  ...
  function renderMetalEstimate(policy, container) {
    ...
    if (!hasAlloc || ewAmount <= 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" class="small">Set allocation and deposit EW to see an estimate.</td></tr>';
    ...
    const est =
      policy.currentEstimate || window.EWIns.calcEstimate(ewAmount, alloc, []);
...
<td>${leftEW > 0 ? `<span class="uninsured-warn">${leftEW.toFixed(0)}</span>` : "&mdash;"} EW</td>`;
    ...
    if (totalUnins)
      totalUnins.textContent = tu > 0 ? tu.toFixed(0) + " EW" : "&mdash;";
  }
  ...
  function updateDepositEstimatePreview_(inner, policy, depositUnits) {
    ...
    const allocMismatch =
      JSON.stringify(savedAlloc) !== JSON.stringify(liveAlloc);
    if (mismatchEl) {
    ...
    function doRender(items) {
      ...
      if (!hasAlloc) {
        previewEl.innerHTML =
          '<span class="small" style="color:#888;">Save an allocation to see estimate preview.</span>';
        return;
      }

      const calcRow = (ewAmt) => {
        if (ewAmt <= 0)
          return { metals: {}, totalInsuredEW: 0, totalLeftoverEW: 0 };
 ...
 <th>Total EW</th>
  ...
  <tbody>
    ...
    if (!priceItems || !priceItems.length) {
      previewEl.innerHTML =
        '<span class="small" style="color:#888;">Loading prices...</span>';
  ...
  function renderWithdrawSection(policy, container) {
    ...
    if (metalsWdSection) {
      metalsWdSection.style.display =
        activity === "Active" && !isPending ? "" : "none";
    }
    ...
    function update() {
      ...
      if (valEl)
        valEl.textContent = `You receive: ${units} x 500 = ${units * 500} EW`;
    }
  ...
  function renderPendingSection(policy, container) {
    ...
    if (banner) {
      const submitted = temp.submittedAt
        ? new Date(temp.submittedAt).toLocaleString()
        : "?";
...
<!-- Deposit section -->
 ...
 <p class="small" style="margin:0 0 8px;">A policy can only be deleted when it is Empty (no units stored, no pending requests).</p>
  ...
  function renderPolicies(arr) {
    ...
    if (!arr || !arr.length) {
      list.innerHTML =
        '<div class="small" style="margin-top:10px;">No policies yet. Click "+ Create New Policy" to get started.</div>';
      updateCreateBtn();
      return;
    }
...
<td><button class="btn-more-info" type="button">More Info &#x25BC;</button></td>`;
  ...
  window.EWIns.renderAll = function () {
    renderPolicies(window.EWIns.state.policies);
  };
