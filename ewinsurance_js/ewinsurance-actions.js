...
(function () {
  ...
  function idToken() {
    return window.EWIns.state.idToken;
  }
  ...
  async function renamePolicy(inner) {
    ...
    if (!validatePolicyName_(name)) {
      if (notifier) {
        notifier.textContent =
          "Error: Name may only contain letters, numbers and spaces (max 40 chars).";
        notifier.style.color = "#c00";
      } else
        alert(
          "Policy name may only contain letters, numbers and spaces (max 40 characters).",
        );
      return;
    }

    if (notifier) {
      notifier.textContent = "Now saving...";
      notifier.style.color = "#888";
    }
    ...
    try {
      ...
      if (notifier) {
        notifier.textContent = "Now saved as " + name;
        notifier.style.color = "#155724";
      }
      ...
      if (notifier) {
        notifier.textContent = "Error: " + e.message;
        notifier.style.color = "#c00";
  ...
  async function saveAllocation(inner) {
    ...
    if (sum > 100) {
      if (notifier) {
        notifier.textContent = "Error: Allocation exceeds 100%.";
        notifier.style.color = "#c00";
    ...
    try {
      ...
      if (notifier) {
        notifier.textContent = "Now saved: " + (summary || "(empty)");
        notifier.style.color = "#155724";
      }
      await window.EWIns.loadPolicies();
      window.EWIns.renderAll();
  ...
  async function requestDeposit(inner) {
    ...
    if (balance < cost) {
      alert(
        `Insufficient balance. You need ${cost} EW but have ${balance} EW.`,
      );
      return;
    }
  ...
  async function requestWithdraw(inner) {
    ...
    const stored = safeJsonParse(
      (
        window.EWIns.state.policies.find(
  ...
  async function requestWithdrawMetals(inner) {
    ...
    if (
      !confirm(
        "Are you sure? Your EW balance will NOT be credited. This closes the insured position after admin approval.",
      )
    )
      return;
    try {
  ...
  async function deletePolicy(inner) {
    const insuranceId = inner.dataset.id;
    if (!confirm("Permanently delete this policy? This cannot be undone."))
      return;
    try {
      await window.apiPost("insuranceDelete", {
        idToken: idToken(),
        insuranceId,
      });
      await reloadPage_();
  ...
  function wireInner_(inner) {
    inner
      ...
      ?.addEventListener("click", () => renamePolicy(inner));
    inner
      .querySelector(".btn-save-alloc")
      ?.addEventListener("click", () => saveAllocation(inner));
    inner
      .querySelector(".btn-deposit")
      ?.addEventListener("click", () => requestDeposit(inner));
    inner
      .querySelector(".btn-withdraw")
      ?.addEventListener("click", () => requestWithdraw(inner));
    inner
      .querySelector(".btn-withdraw-metals")
      ?.addEventListener("click", () => requestWithdrawMetals(inner));
    inner
      .querySelector(".btn-cancel-pending")
      ?.addEventListener("click", () => cancelPending(inner));
    inner
      .querySelector(".btn-delete-policy")
      ?.addEventListener("click", () => deletePolicy(inner));
  }
