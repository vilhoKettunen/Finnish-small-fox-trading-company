...
(function () {
    ...
    return (
      (window.items || []).find(
        (i) =>
          ...
          String(currencyName).toLowerCase(),
      ) || null
    );
  }
  ...
  function formatEwWithDisplayCurrencyBreakdown_(ewAmount, currencyName, opts) {
    ...
    if (!currencyName || currencyName === base)
      return `${ew.toFixed(2)} ${base}`;
    ...
    if (excess >= EXCESS_THRESHOLD_EW_)
      breakdown += `, Excess EW ${excess.toFixed(2)}`;
    breakdown += ")";

    return `${ew.toFixed(2)} ${base} ${breakdown}`;
  }

  window.quickStackCalc =
    window.quickStackCalc ||
    function quickStackCalc() {
      ...
      if (!item) return;
      const buyText = item.buyStack
        ? `${qty} × ${item.bundleSize} ${item.name} buys for ${(qty * item.buyStack).toFixed(2)} EW`
        : "";
      const sellText = item.sellStack
        ? `${qty} × ${item.bundleSize} ${item.name} sells for ${(qty * item.sellStack).toFixed(2)} EW`
        : "";
      document.getElementById("quickStackResult").textContent = [
        buyText,
        sellText,
      ]
        .filter(Boolean)
  ...
  window.quickIndivCalc =
    ...
    function quickIndivCalc() {
      ...
      const perEachBuy =
        item.buyEach != null && isFinite(Number(item.buyEach))
          ? Number(item.buyEach)
          : item.buyStack != null && isFinite(Number(item.buyStack))
            ? Number(item.buyStack) / bundleSize
            : null;

      const perEachSell =
        item.sellEach != null && isFinite(Number(item.sellEach))
          ? Number(item.sellEach)
          : item.sellStack != null && isFinite(Number(item.sellStack))
            ? Number(item.sellStack) / bundleSize
            : null;

      const buyText =
        perEachBuy != null
          ? `${qty} × ${item.name} buys for ${(qty * perEachBuy).toFixed(2)} EW`
          : "";
      const sellText =
        perEachSell != null
          ? `${qty} × ${item.name} sells for ${(qty * perEachSell).toFixed(2)} EW`
          : "";
      document.getElementById("quickIndivResult").textContent = [
        buyText,
        sellText,
      ]
        .filter(Boolean)
  ...
  window.addBuyAccountBalance =
    window.addBuyAccountBalance ||
    function addBuyAccountBalance() {
      const balance =
        parseFloat(document.getElementById("buyAccountBalanceInput").value) ||
        0;
      if (balance <= 0) return;
      window.buyCart.push({
  ...
  window.addCustomBuyStack =
    window.addCustomBuyStack ||
    function addCustomBuyStack() {
      const name = document.getElementById("customBuyStackSelect").value;
      const qty =
        parseInt(
          document.getElement.getElementById("customBuyStackQty").value,
        ) || 0;
      const price =
        parseFloat(document.getElementById("customBuyStackPrice").value) || 0;
      const item = (window.items || []).find((i) => i.name === name);
  ...
  window.addCustomBuyIndividual =
    window.addCustomBuyIndividual ||
    function addCustomBuyIndividual() {
      const name = document.getElementById("customBuyIndivSelect").value;
      const qty =
        parseInt(document.getElementById("customBuyIndivQty").value) || 0;
      const price =
        parseFloat(document.getElementById("customBuyIndivPrice").value) || 0;
      const item = (window.items || []).find((i) => i.name === name);
      if (!item || qty <= 0 || price <= 0) return;
      window.buyCart.push({
        name: item.name,
  ...
  window.addFullyCustomBuy =
    ...
    function addFullyCustomBuy() {
      ...
      const stackSize =
        parseInt(document.getElementById("customBuyStackSize").value) || 1;
      const price =
        parseFloat(document.getElementById("customBuyPrice").value) || 0;
      if (!name || qty <= 0 || price <= 0 || stackSize <= 0) return;
      window.buyCart.push({
        name,
        qty,
        bundleSize: stackSize,
        price,
        isFullyCustom: true,
        source: "CUSTOM",
      });
  ...
  window.addAccountBalance =
    window.addAccountBalance ||
    function addAccountBalance() {
      const balance =
        parseFloat(document.getElementById("accountBalanceInput").value) || 0;
      if (balance <= 0) return;
      window.sellCart.push({
        name: "Account Balance",
        qty: 1,
  ...
  window.addCustomSellStack =
    window.addCustomSellStack ||
    function addCustomSellStack() {
      const name = document.getElementById("customSellStackSelect").value;
      const qty =
        parseInt(document.getElementById("customSellStackQty").value) || 0;
      const price =
        parseFloat(document.getElementById("customSellStackPrice").value) || 0;
      const item = (window.items || []).find((i) => i.name === name);
      if (!item || qty <= 0 || price <= 0) return;
      window.sellCart.push({
        name: item.name,
  ...
  window.addCustomSellIndividual =
    window.addCustomSellIndividual ||
    function addCustomSellIndividual() {
      const name = document.getElementById("customSellIndivSelect").value;
      const qty =
        parseInt(document.getElementById("customSellIndivQty").value) || 0;
      const price =
        parseFloat(document.getElementById("customSellIndivPrice").value) || 0;
      const item = (window.items || []).find((i) => i.name === name);
      if (!item || qty <= 0 || price <= 0) return;
      window.sellCart.push({
        name: item.name,
  ...
  window.addFullyCustomSell =
    ...
    function addFullyCustomSell() {
      ...
      const stackSize =
        parseInt(document.getElementById("customSellStackSize").value) || 1;
      const price =
        parseFloat(document.getElementById("customSellPrice").value) || 0;
      if (!name || qty <= 0 || price <= 0 || stackSize <= 0) return;
      window.sellCart.push({
        name,
        qty,
  ...
  /* ===== Row coloring ===== */
  window.evaluateRowColorBuy =
    window.evaluateRowColorBuy ||
    function evaluateRowColorBuy(entry) {
      const item = (window.items || []).find((i) => i.name === entry.name);
      if (!item) return null;
      if (entry.source === "OCM") return null;
      if (entry.isFullyCustom) return null;

      const b = Number(item.bundleSize || 1) || 1;

      // Sum ALL buy-cart entries for this item (not just this single entry)
      // This catches both "Full Stacks (Store)" and "Individual Items (Store)"
      // being added separately for the same item name.
      const buyList = window.buyCart || [];
      ...
      buyList.forEach((e) => {
        if (
          ...
          totalRequestedPieces += e.bundleSize
            ? Number(e.qty || 0) * e.bundleSize
            : Number(e.qty || 0);
        }
      });

      // Use individual pieces (column H) as stock base
      const stockPieces = Number(item.indivStock || 0);
      const availPercent = Math.max(
        0,
        Math.min(100, Number(item.availPercent || 0)),
      );
      const availablePieces = stockPieces * (availPercent / 100);

      if (stockPieces <= 0) return "row-warning-red";
      if (totalRequestedPieces > stockPieces) return "row-warning-red";
      if (totalRequestedPieces > availablePieces) return "row-warning-yellow";
      return null;
    };

  window.evaluateRowColorSell =
    window.evaluateRowColorSell ||
    function evaluateRowColorSell(entry) {
      const item = (window.items || []).find((i) => i.name === entry.name);
      if (!item || entry.source === "OCM") return null;

      const b = Number(item.bundleSize || 1) || 1;
      const targetStacks = Number(item.targetEach || 0);
      if (targetStacks <= 0) return null;

      // Current stock in stacks = indivStock (pieces) / bundleSize
      const currentStacks = Number(item.indivStock || 0) / b;

      // Sum ALL sell list entries for this item (not just this single entry)
      const sellList = window.sellCart || [];
      let totalSellingStacks = 0;
      sellList.forEach((e) => {
        if (e.name === entry.name && e.source !== "OCM") {
          totalSellingStacks += e.bundleSize
            ? Number(e.qty || 0)
            : Number(e.qty || 0) / b;
        }
      });

      // existing stock + all items being sold vs target
      const totalProjected = currentStacks + totalSellingStacks;
      if (totalProjected > targetStacks) return "row-warning-red";
      return null;
    };
  ...
  window.renderBuyList =
    ...
    function renderBuyList() {
      ...
      (window.buyCart || []).forEach((e) => {
        ...
        if (e.isBalance) {
          rowText = `Account Balance: ${rowTotal.toFixed(2)} EW`;
          if (e.isAccountBalancePinned) rowText += " (auto)";
        }
        total += rowTotal;
        ...
        if (curr && curr !== window.BASE_CURRENCY)
          rowText += " " + window.formatValue(rowTotal, curr, true);
      ...
      document.getElementById("buyTotal").textContent =
        formatEwWithDisplayCurrencyBreakdown_(total, curr, {
          ratePerEachEw,
          roundMode: "CEIL",
        });
  ...
  window.renderSellList =
    ...
    function renderSellList() {
      ...
      (window.sellCart || []).slice().forEach((e) => {
        ...
        if (e.isBalance && !e.isAccountBalancePinned) {
          rowText = `Account Balance (manual): ${rowTotal.toFixed(2)} EW`;
        }
        total += rowTotal;
        ...
        if (curr && curr !== window.BASE_CURRENCY)
          rowText += " " + window.formatValue(rowTotal, curr, false);
  ...
  window.calculateNet =
    ...
    function calculateNet() {
      const buyTotalRaw = (window.buyCart || []).reduce((sum, e) => {
        ...
        let rowTotal;
        if (e.isBalance)
          rowTotal = (Number(e.price) || 0) * (Number(e.qty) || 1);
        else if (e.bundleSize && e.isFullyCustom)
          rowTotal =
            (Number(e.qty || 0) / Number(e.bundleSize || 1)) *
            Number(e.price || 0);
  ...
  window.updateCombinedTotals =
    ...
    function updateCombinedTotals() {
      ...
      if (abEl) {
        ...
        let balText = formatEwWithDisplayCurrencyBreakdown_(
      ...
      if (netSpan) {
        if (netRaw >= 0) {
          ...
          netSpan.textContent = formatEwWithDisplayCurrencyBreakdown_(
            netRaw,
            curr,
            {
      ...
      if (afterEl) {
        ...
        if (after >= 0) {
          afterEl.textContent = formatEwWithDisplayCurrencyBreakdown_(
            after,
            curr,
            {
              ratePerEachEw: rateSell,
