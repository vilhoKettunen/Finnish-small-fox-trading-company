...
issuanceShowDestroyed: true,
    ...
    function renderWealthDist() {
        ...
        BankCharts.drawWealthDistChart('bankWealthChart', filtered.map(r => r.ts), { top1, next9, next20, bot70 }, state.wealthMode, state.range);

        // Populate date picker with DD/MM/YYYY display
    ...
    function wireIntroGuide() {
        ...
        btnGuide.addEventListener('click', () => {
            ...
            // Swap the arrow character
            btnGuide.innerHTML = btnGuide.innerHTML
                .replace(/[▶▼►]/u, !isOpen ? '\u25BC' : '\u25BA')
                .replace(/[&#9658;&#9660;]/g, !isOpen ? '&#9660;' : '&#9658;');
            // Re-set cleanly
            const arrow = !isOpen ? '\u25BC' : '\u25BA';
    ...
    function wireVelocitySection() {
        const header = $('velInflHeader');
        if (!header) return;

        // Click on header but NOT on the info button
        header.addEventListener('click', async (e) => {
            if (e.target.closest('#velInfoBtn')) return; // let the info button handle itself
            const content = $('velInflContent');
            if (!content) return;
            const isOpen = content.style.display !== 'none';
            content.style.display = isOpen ? 'none' : 'block';
            header.setAttribute('aria-expanded', String(!isOpen));
            const arrow = $('velCollapseArrow');
  ...
  if (state.velLoaded) renderVelocityItem();
    ...
    function updateVelToggleBLabels() {
        ...
        // Update label text for A and B in both charts
        const itemLabelA = $('bankVelItemLabelA'); if (itemLabelA) itemLabelA.childNodes[itemLabelA.childNodes.length - 1].nodeValue = ` Show ${a}`;
        ...
        if (!b) {
            // Hide B toggles for single-series metrics (ocmVolume)
    ...
    // ─── Velocity rendering ───────────────────────────────────────────────────
    let _velRows = null; // cached for current metric
...
.slice(0, 10)
 ...
 : 'Showing top 10 items (no selection \u2014 use the item selector above)';
    ...
    function renderVelocityCombined() {
        ...
        function sumSeries(colKey) {
            return allDates.map(dk => {
                ...
                const s = day.reduce((acc, r) => acc + (isFinite(r[colKey]) ? r[colKey] : 0), 0);
                return s;
            });
        }
    ...
    function wireInfoModal() {
        // Wire all [data-bank-info] buttons including the velocity one
        document.querySelectorAll('[data-bank-info]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation(); // prevent velocity collapse-header from firing
